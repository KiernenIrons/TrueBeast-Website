...
import { GameState, Building, Upgrade, Achievement } from './types';
...
export function getBuildingCost(building: Building, owned: number, qty: number = 1): number {
...
export function getMaxAffordable(building: Building, owned: number, clout: number): { qty: number; cost: number } {
    let qty = 0;
    let cost = 0;
    while (true) {
...
const BUILDING_UPGRADE_TIERS: string[][] = [
    // viewer
...
function generateUpgrades(): Upgrade[] {
    ...
    // 1. Building tier upgrades (15 buildings x 8 tiers = 120)
    ...
    BUILDINGS.forEach((b, bi) => {
        thresholds.forEach((thresh, ti) => {
            ...
            ups.push({
                ...
                buildingId: b.id,
                multiplier: 2,
                condition: (s: GameState) => (s.buildings[b.id] || 0) >= thresh,
            });
        });
    });
    ...
    clickNames.forEach((name, i) => {
        ups.push({
            ...
            type: 'click_mult',
            multiplier: 2,
            condition: (s: GameState) => s.clicks >= clickThresh[i],
        });
    });
    ...
    goldenNames.forEach((name, i) => {
        ups.push({
            ...
            multiplier: 1.1,
            condition: (s: GameState) => s.goldenCloutClicks >= goldenThresh[i],
        });
    });

    // 4. Total clout milestone upgrades (10)
    const milestoneData: { name: string; cost: number; thresh: number; emoji: string; desc: string }[] = [
    ...
    milestoneData.forEach((m, i) => {
        ups.push({
            ...
            type: 'global_mult',
            multiplier: 1.05,
            condition: (s: GameState) => s.totalCloutEver >= m.thresh,
        });
    });

    // 5. Prestige upgrades (10 — purchasable with Viral Chips)
    const prestigeUps: { id: string; name: string; desc: string; cost: number; emoji: string; type: Upgrade['type']; multiplier: number }[] = [
        ...
        { id: 'prestige_8', name: 'God of Clout',         desc: 'Click power x4.',                   cost: 30,  emoji: '🌟', type: 'click_mult',  multiplier: 4 },
    ...
    const synergyData: { a: string; b: string; name: string; desc: string; thresh: number; cost: number }[] = [
    ...
    synergyData.forEach((syn, i) => {
        ups.push({
            ...
            buildingB: syn.b,
            synergyBonus: 0.01,
            condition: (s: GameState) => (s.buildings[syn.a] || 0) >= syn.thresh && (s.buildings[syn.b] || 0) >= 1,
        });
    });
...
function generateAchievements(): Achievement[] {
    ...
    const cloutMilestones: [number, string, string, string][] = [
    ...
    cloutMilestones.forEach(([thresh, name, desc, icon]) => {
        achs.push({
            ...
            name, desc, icon,
            category: 'clout',
            condition: (s: GameState) => s.totalCloutEver >= thresh,
        });
    });

    // B. CPS milestones (18)
    const cpsMilestones: [number, string, string, string][] = [
    ...
    cpsMilestones.forEach(([thresh, name, desc, icon]) => {
        achs.push({
            ...
            name, desc, icon,
            category: 'cps',
            condition: (s: GameState) => s.cps >= thresh,
        });
    });

    // C. Click count milestones (14)
    const clickMilestones: [number, string, string, string][] = [
    ...
    clickMilestones.forEach(([thresh, name, desc, icon]) => {
        achs.push({
            ...
            name, desc, icon,
            category: 'clicks',
            condition: (s: GameState) => s.clicks >= thresh,
        });
    });

    // D. Per-building ownership achievements (15 buildings x 11 tiers = 165)
    const buildingOwnershipTiers = [1, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500];
    const buildingAchNames: Record<string, string[]> = {
    ...
    const goldenMilestones: [number, string, string, string][] = [
    ...
    goldenMilestones.forEach(([thresh, name, desc, icon]) => {
        achs.push({
            ...
            name, desc, icon,
            category: 'golden',
            condition: (s: GameState) => s.goldenCloutClicks >= thresh,
        });
    });

    // F. Prestige achievements (15)
    const prestigeMilestones: [number, string, string, string][] = [
    ...
    prestigeMilestones.forEach(([thresh, name, desc, icon]) => {
        achs.push({
            ...
            name, desc, icon,
            category: 'prestige',
            condition: (s: GameState) => (s.prestigeLevel || 0) >= thresh,
        });
    });

    // G. Speed run achievements (10)
    const speedAchs: { id: string; name: string; desc: string; icon: string; thresh: number; time: number }[] = [
    ...
    speedAchs.forEach(a => {
        achs.push({
            ...
            icon: a.icon,
            category: 'speed',
            condition: (s: GameState) => s.totalCloutEver >= a.thresh && s.timePlayed <= a.time,
        });
    });

    // H. Special / misc achievements (50+)
    const specialAchs: { id: string; name: string; desc: string; icon: string; shadow?: boolean; condition: (s: GameState) => boolean }[] = [
        // Shadow / secret
        { id: 'shadow_bot', name: 'Not a Bot',         desc: 'Click 10,000 times in a single session. Totally human.', icon: '🤖', shadow: true, condition: (s: GameState) => s.sessionClicks >= 10000 },
        { id: 'shadow_insomniac', name: 'Insomniac',   desc: 'Play for 24 hours total.', icon: '😴', shadow: true, condition: (s: GameState) => s.timePlayed >= 86400 },
        { id: 'shadow_humble', name: 'Humble',          desc: 'Have zero buildings for 1 minute after buying one.', icon: '🥺', shadow: true, condition: (s: GameState) => s.humbleTimer >= 60 },
        { id: 'shadow_fast_gold', name: 'Golden Touch', desc: 'Click a Golden Clout within 1 second of it appearing.', icon: '⚡', shadow: true, condition: (s: GameState) => s.goldenFastClick === true },
        { id: 'shadow_broke', name: 'Broke Creator',   desc: 'Spend all your Clout and have 0 left.', icon: '💸', shadow: true, condition: (s: GameState) => s.clout < 1 && s.totalCloutEver >= 100 },
        { id: 'shadow_noclick', name: 'Let It Cook',   desc: 'Don\'t click the controller for 5 minutes while CPS > 0.', icon: '🍳', shadow: true, condition: (s: GameState) => s.afkTimer >= 300 && s.cps > 0 },
        { id: 'shadow_refund', name: 'Return Policy',  desc: 'Start the game fresh after earning over 1 billion Clout.', icon: '🔄', shadow: true, condition: (s: GameState) => s.prestigeLevel >= 1 && s.totalCloutEver >= 1e9 },
        { id: 'shadow_night', name: 'Night Owl',       desc: 'Play between midnight and 4am (local time).', icon: '🦉', shadow: true, condition: (s: GameState) => { const h = new Date().getHours(); return h >= 0 && h < 4 && s.timePlayed > 60; } },
        { id: 'shadow_frenzy_chain', name: 'Frenzy Chain', desc: 'Get 10 Golden Clouts in one prestige run.', icon: '⛓️', shadow: true, condition: (s: GameState) => s.goldenCloutThisRun >= 10 },
        { id: 'shadow_og', name: 'Day One',            desc: 'Play for 30 days total (play time).', icon: '🏅', shadow: true, condition: (s: GameState) => s.timePlayed >= 2592000 },

        // Combo / hoarder
        { id: 'hoarder', name: 'Hoarder', desc: 'Own 100 of every building simultaneously.', icon: '📦', condition: (s: GameState) => BUILDINGS.every(b => (s.buildings[b.id] || 0) >= 100) },
        { id: 'hoarder_200', name: 'Super Hoarder', desc: 'Own 200 of every building simultaneously.', icon: '🏰', condition: (s: GameState) => BUILDINGS.every(b => (s.buildings[b.id] || 0) >= 200) },
        { id: 'hoarder_250', name: 'Warehouse God', desc: 'Own 250 of every building simultaneously.', icon: '🌐', condition: (s: GameState) => BUILDINGS.every(b => (s.buildings[b.id] || 0) >= 250) },
        { id: 'upgrade_10', name: 'Upgrade Beginner', desc: 'Purchase 10 upgrades.', icon: '🔧', condition: (s: GameState) => s.upgrades.size >= 10 },
        { id: 'upgrade_50', name: 'Upgrade Collector', desc: 'Purchase 50 upgrades.', icon: '🛠️', condition: (s: GameState) => s.upgrades.size >= 50 },
        { id: 'upgrade_100', name: 'Upgrade Master', desc: 'Purchase 100 upgrades.', icon: '⚙️', condition: (s: GameState) => s.upgrades.size >= 100 },
        { id: 'upgrade_150', name: 'Upgrade Legend', desc: 'Purchase 150 upgrades.', icon: '🔩', condition: (s: GameState) => s.upgrades.size >= 150 },
        { id: 'upgrade_180', name: 'Upgrade God', desc: 'Purchase every upgrade available.', icon: '♾️', condition: (s: GameState) => s.upgrades.size >= UPGRADES.length - 5 },

        // First buys
        { id: 'first_building', name: 'First Investment', desc: 'Buy your first building.', icon: '🏗️', condition: (s: GameState) => Object.values(s.buildings).reduce((a, b) => a + b, 0) >= 1 },
        { id: 'first_upgrade', name: 'First Upgrade', desc: 'Purchase your first upgrade.', icon: '🆙', condition: (s: GameState) => s.upgrades.size >= 1 },
        { id: 'ten_buildings', name: 'Small Operation', desc: 'Own 10 buildings total across all types.', icon: '🏢', condition: (s: GameState) => Object.values(s.buildings).reduce((a, b) => a + b, 0) >= 10 },
        { id: '100_buildings', name: 'Content Empire', desc: 'Own 100 buildings total.', icon: '🏙️', condition: (s: GameState) => Object.values(s.buildings).reduce((a, b) => a + b, 0) >= 100 },
        { id: '500_buildings', name: 'Megacorp', desc: 'Own 500 buildings total.', icon: '🌆', condition: (s: GameState) => Object.values(s.buildings).reduce((a, b) => a + b, 0) >= 500 },
        { id: '1000_buildings', name: 'Infinite Corporation', desc: 'Own 1,000 buildings total.', icon: '🌇', condition: (s: GameState) => Object.values(s.buildings).reduce((a, b) => a + b, 0) >= 1000 },
        { id: '5000_buildings', name: 'God Corp', desc: 'Own 5,000 buildings total.', icon: '🌃', condition: (s: GameState) => Object.values(s.buildings).reduce((a, b) => a + b, 0) >= 5000 },

        // Frenzy related
        { id: 'frenzy_1', name: 'Frenzy Mode', desc: 'Activate your first Frenzy from a Golden Clout.', icon: '🌀', condition: (s: GameState) => s.frenzyCount >= 1 },
        { id: 'frenzy_7', name: 'Lucky Frenzy', desc: 'Activate 7 Frenzies.', icon: '🎯', condition: (s: GameState) => s.frenzyCount >= 7 },
        { id: 'frenzy_77', name: 'Frenzy Addict', desc: 'Activate 77 Frenzies.', icon: '🔥', condition: (s: GameState) => s.frenzyCount >= 77 },
        { id: 'click_frenzy', name: 'Click Storm', desc: 'Trigger your first Click Frenzy.', icon: '👆', condition: (s: GameState) => s.clickFrenzyCount >= 1 },
        { id: 'free_up', name: 'Free Lunch', desc: 'Receive a free upgrade from a Golden Clout.', icon: '🎁', condition: (s: GameState) => s.freeUpgradeCount >= 1 },

        // Offline
        { id: 'offline_1h', name: 'AFK Creator', desc: 'Return after being away for 1 hour and earn offline income.', icon: '💤', condition: (s: GameState) => s.longestOffline >= 3600 },
        { id: 'offline_8h', name: 'Passive Income', desc: 'Return after 8 hours away.', icon: '😴', condition: (s: GameState) => s.longestOffline >= 28800 },
        { id: 'offline_24h', name: 'Overnight Creator', desc: 'Return after 24 hours away.', icon: '🌙', condition: (s: GameState) => s.longestOffline >= 86400 },

        // Leaderboard / social
        { id: 'logged_in', name: 'Community Member', desc: 'Sign in to save your progress.', icon: '🔑', condition: (s: GameState) => s.isLoggedIn === true },
        { id: 'leaderboard_top25', name: 'Top Contender', desc: 'Appear on the global leaderboard.', icon: '📋', condition: (s: GameState) => s.isLoggedIn === true && s.totalCloutEver >= 1000 },

        // Fun / misc
        { id: 'cps_click_equal', name: 'Balanced Creator', desc: 'Have your click power equal to your CPS (within 10%).', icon: '⚖️', condition: (s: GameState) => s.cps > 0 && Math.abs(s.clickPower - s.cps) / s.cps <= 0.1 },
        { id: 'all_buildings', name: 'Full Roster', desc: 'Own at least 1 of every building type.', icon: '✅', condition: (s: GameState) => BUILDINGS.every(b => (s.buildings[b.id] || 0) >= 1) },
        { id: 'wealthy', name: 'Sitting on Clout', desc: 'Have 1 trillion Clout banked (unspent) at once.', icon: '🤑', condition: (s: GameState) => s.clout >= 1e12 },
        { id: 'ultra_wealthy', name: 'Obscene Wealth', desc: 'Have 1 quadrillion Clout banked at once.', icon: '💰', condition: (s: GameState) => s.clout >= 1e15 },
        { id: 'no_upgrades_1m', name: 'Purist', desc: 'Reach 1 million Clout with no upgrades purchased.', icon: '🏴', condition: (s: GameState) => s.totalCloutEver >= 1e6 && s.upgrades.size === 0 },
        { id: 'early_prestige', name: 'Speedrunner Prestige', desc: 'Prestige within 1 hour of starting a new run.', icon: '🏁', condition: (s: GameState) => s.prestigeLevel >= 1 && s.timeSincePrestige <= 3600 },
        { id: 'chip_hoarder', name: 'Chip Collector', desc: 'Accumulate 100 Viral Chips.', icon: '🧩', condition: (s: GameState) => (s.viralChips || 0) >= 100 },
        { id: 'chip_god', name: 'Chip God', desc: 'Accumulate 1,000 Viral Chips.', icon: '🌟', condition: (s: GameState) => (s.viralChips || 0) >= 1000 },
        { id: 'ten_prestige_ups', name: 'Prestige Shopper', desc: 'Buy 5 prestige upgrades.', icon: '🛒', condition: (s: GameState) => [...s.upgrades].filter(id => id.startsWith('prestige_')).length >= 5 },
        { id: 'all_prestige_ups', name: 'Prestige Complete', desc: 'Buy all prestige upgrades.', icon: '🎖️', condition: (s: GameState) => [...s.upgrades].filter(id => id.startsWith('prestige_')).length >= 10 },
        { id: 'save_loaded', name: 'Persistence Pays', desc: 'Load a save game.', icon: '💾', condition: (s: GameState) => s.saveLoaded === true },
    ];
